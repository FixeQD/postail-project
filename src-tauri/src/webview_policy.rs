...
// ---------------------------------------------------------------------------
// Windows - WebView2 resource handler
// ---------------------------------------------------------------------------

/// Registers a `WebResourceRequested` handler on the isolated `ICoreWebView2` created by `cmd::webview`
...
pub fn register_webview2_resource_handler(
    ...
    let handler = WebResourceRequestedEventHandler::create(Box::new(
        move |_wv, args: Option<ICoreWebView2WebResourceRequestedEventArgs>| {
            ...
            let response = match crate::protocol::handle_message_route(&app, path, query) {
                Some(resp) => resp,
                None => {
