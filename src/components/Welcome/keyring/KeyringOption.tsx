		...
		<button
			...
			className={`group relative flex h-full w-full flex-col justify-between overflow-hidden rounded-3xl p-6 text-left transition-all duration-300 focus:ring-2 focus:ring-offset-2 focus:ring-offset-[var(--app-bg)] focus:outline-none ${
				isDisabled
					...
					: 'cursor-pointer border border-[var(--border-faint)] bg-[var(--surface-glass)] hover:-translate-y-1 hover:border-status-info/30 hover:bg-[var(--surface-panel)] hover:shadow-xl'
			}`}
			...
			{available && !disabled && (
				<div className='pointer-events-none absolute -inset-px rounded-3xl opacity-0 transition-opacity duration-500 group-hover:opacity-100'>
					...
					<div className='absolute -top-20 -right-20 h-40 w-40 rounded-full bg-status-info/15 blur-3xl transition-opacity group-hover:opacity-100' />
				</div>
			)}
			...
			<div className='relative'>
				...
				<div className='absolute top-0 right-0 flex flex-col items-end gap-1.5'>
					{loading ? (
						<div className='flex items-center rounded-full bg-[var(--surface-active)] px-2.5 py-1 text-[11px] font-bold tracking-wider text-[var(--text-tertiary)] uppercase ring-1 ring-[var(--border-subtle)]'>
							<div className='mr-1.5 h-3.5 w-3.5 animate-spin rounded-full border-2 border-[var(--surface-active)] border-t-[var(--text-secondary)]' />
							{t('common:status.loading')}
						</div>
					) : available ? (
						<div className='flex items-center rounded-full bg-status-info/15 px-2.5 py-1 text-[11px] font-bold tracking-wider text-status-info uppercase ring-1 ring-status-info/30'>
				...
				<div
					className={`mb-5 flex h-14 w-14 items-center justify-center rounded-2xl ring-1 transition-all duration-300 ${
						available
							? 'bg-status-info/15 ring-status-info/30 group-hover:scale-110 group-hover:bg-status-info/15 group-hover:shadow-lg'
							: 'bg-[var(--surface-active)] ring-[var(--border-subtle)]'
					}`}>
					<Key
						className={`h-7 w-7 transition-colors duration-300 ${available ? 'text-status-info' : 'text-[var(--text-tertiary)]'}`}
					/>
				</div>
			...
			<div className='relative mt-auto'>
				...
				{available && !loading && (
					<div className='text-muted-foreground mt-2 flex items-center text-xs'>
						<ShieldCheck className='mr-1.5 h-3.5 w-3.5 text-status-info' />
						System integration
					</div>
				)}

				{loading && (
					<div className='mt-2 flex items-center text-xs font-medium text-status-info'>
						<div className='mr-2 h-3.5 w-3.5 animate-spin rounded-full border-2 border-status-info border-t-transparent' />
						Initializing...
					</div>
				)}
			</div>
