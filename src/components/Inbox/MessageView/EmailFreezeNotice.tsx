...
export const EmailFreezeNotice: React.FC<EmailFreezeNoticeProps> = ({ stats, onDismiss }) => {
	...
	return (
		<div className='relative flex items-center justify-between gap-4 border-b border-amber-200/50 bg-amber-50/80 px-4 py-3 dark:border-amber-900/30 dark:bg-amber-900/10'>
			...
			<div className='flex items-center gap-2'>
				<Button
					...
					size='sm'
					onClick={onDismiss}
					className='h-8 text-xs text-amber-700 hover:bg-amber-100 hover:text-amber-900 dark:text-amber-500 dark:hover:bg-amber-900/50 dark:hover:text-amber-400'>
					{t('messageView.freezeNotice.readAsStatic')}
				</Button>
				<Button
					size='sm'
					onClick={handleResume}
					className='h-8 border-0 bg-amber-500 text-xs text-white hover:bg-amber-600 dark:bg-amber-600 dark:hover:bg-amber-500'>
					{t('messageView.freezeNotice.resume')}
				</Button>
				<button
					onClick={onDismiss}
					className='ml-2 flex h-8 w-8 items-center justify-center rounded-md text-amber-500 hover:bg-amber-100 dark:hover:bg-amber-900/50'>
					<X className='h-4 w-4' />
				</button>
			</div>
		</div>
