...
export default function DragDropPlugin(): React.ReactNode {
	...
	useEffect(() => {
		...
		const handleDragOver = (e: DragEvent) => {
			...
			if (dragCounter.current > 0) {
				...
				} else if (attachRect && e.clientX >= attachRect.left && e.clientX <= attachRect.right) {
		...
		const handleDrop = (e: DragEvent) => {
			...
			// We process the drop regardless of isDragging state 
			// because dragleave might have fired prematurely on some Linux DEs
			setIsDragging(false)
			dragCounter.current = 0
			...
			console.log(`[DragDrop] Processing. Mode: ${mode}, Files: ${files.length}, URIs: ${uris.length}`)
			handleFileProcessing(files, uris, mode)
			setActiveZone(null)
		}
	...
	const handleFileProcessing = async (
		...
		try {
			...
			for (const uri of uris) {
				...
				if (mode === 'inline' && attachment.contentType.startsWith('image/')) {
					insertInlineImage(attachment)
				}
			}
		} catch (err) {
	...
	return (
		<div 
			className={`fixed inset-0 z-[100] flex items-center justify-center bg-zinc-950/80 backdrop-blur-sm p-12 transition-all duration-200 pointer-events-none ${(!isDragging && !isProcessing) ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}
		...
		>
			{isProcessing ? (
				...
				<div className="flex w-full max-w-4xl h-96 gap-8 pointer-events-none">
					{dragType === 'media' ? (
						<>
							<div 
								ref={(el) => { if (el) zonesRef.current.inline = el.getBoundingClientRect() }}
								className={`flex-1 rounded-2xl border-4 border-dashed transition-all duration-200 flex flex-col items-center justify-center gap-4
									${activeZone === 'inline' 
										? 'border-blue-500 bg-blue-500/20 scale-105 shadow-[0_0_30px_rgba(59,130,246,0.3)]' 
										: 'border-zinc-700 bg-zinc-900/40'
									}`}
							>
								<div className={`p-4 rounded-full ${activeZone === 'inline' ? 'bg-blue-500 text-white' : 'bg-zinc-800 text-zinc-400'}`}>
									<ImageIcon size={48} />
								</div>
								<div className="text-center">
									<h3 className={`text-xl font-bold ${activeZone === 'inline' ? 'text-blue-200' : 'text-zinc-300'}`}>Insert Inline</h3>
									<p className="text-zinc-500 mt-1">Embed directly in email</p>
								</div>
							</div>

							<div 
								ref={(el) => { if (el) zonesRef.current.attachment = el.getBoundingClientRect() }}
								className={`flex-1 rounded-2xl border-4 border-dashed transition-all duration-200 flex flex-col items-center justify-center gap-4
							...
							>
								<div className={`p-4 rounded-full ${activeZone === 'attachment' ? 'bg-green-500 text-white' : 'bg-zinc-800 text-zinc-400'}`}>
									<Paperclip size={48} />
								</div>
								<div className="text-center">
									<h3 className={`text-xl font-bold ${activeZone === 'attachment' ? 'text-green-200' : 'text-zinc-300'}`}>Add as Attachment</h3>
									<p className="text-zinc-500 mt-1">Add to file list</p>
								</div>
							</div>
						</>
					) : (
						<div 
							ref={(el) => { if (el) zonesRef.current.attachment = el.getBoundingClientRect() }}
							className={`flex-1 rounded-2xl border-4 border-dashed transition-all duration-200 flex flex-col items-center justify-center gap-6
								${activeZone === 'attachment' 
									? 'border-blue-500 bg-blue-500/20 scale-105 shadow-[0_0_30px_rgba(59,130,246,0.3)]' 
									: 'border-zinc-700 bg-zinc-900/40'
								}`}
						>
							<div className={`p-6 rounded-full ${activeZone === 'attachment' ? 'bg-blue-500 text-white' : 'bg-zinc-800 text-zinc-400'}`}>
								<UploadCloud size={64} />
							</div>
							<div className="text-center">
								<h3 className={`text-2xl font-bold ${activeZone === 'attachment' ? 'text-blue-200' : 'text-zinc-300'}`}>Add Attachment</h3>
								<p className="text-zinc-500 mt-2 text-lg">Drop files to attach</p>
							</div>
						</div>
					)}
				</div>
