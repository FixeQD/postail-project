...
- [ ] 13 - Folders & Organization
    ...
    - [ ] 14.2 - IMAP: Rename folder (RENAME)
    ...
    - [ ] 14.10 - IMAP: Subscribe/unsubscribe folders (SUBSCRIBE/UNSUBSCRIBE)
    - [ ] 14.11 - UI: "Hide folder" option in account settings without unsubscribing

- [ ] 14 - Advanced Search
    - [ ] 15.1 - UI: Advanced search panel (from/to/subject/body/date range/has attachment/folder)
    - [ ] 15.2 - Rust: IMAP SEARCH for server-side search when local FTS isn't enough
    - [ ] 15.3 - UI: Search operators in search field (from:, to:, subject:, before:, after:, has:attachment)
    - [ ] 15.4 - UI: Search history (last 20 queries in localStorage)
    - [ ] 15.5 - UI: Saved searches as virtual mailboxes in sidebar
    - [ ] 15.6 - DB: `saved_searches` table (name, query, icon)
    - [ ] 15.7 - UI: Highlight matching fragments in search results
    - [ ] 15.8 - Rust: Full-text search in message body via FTS5

- [ ] 15 - Templates & Signatures
    - [ ] 16.1 - DB: `signatures` table (id, account_id, name, html_content, is_default)
    - [ ] 16.2 - Rust: CRUD commands for signatures
    - [ ] 16.3 - UI: Signature editor in account settings (same Lexical editor as compose)
    - [ ] 16.4 - UI: Auto-insert default signature on new/reply messages
    - [ ] 16.5 - UI: Signature selector in ComposeScreen (dropdown near footer)
    - [ ] 16.6 - UI: Inline editing of signature in compose
    - [ ] 16.7 - DB: `templates` table (id, account_id, name, subject, html_body)
    - [ ] 16.8 - Rust: CRUD commands for templates
    - [ ] 16.9 - UI: Template gallery accessible from ComposeScreen
    - [ ] 16.10 - UI: "Save as template" from current message
    - [ ] 16.11 - UI: Insert template into compose with edit before sending
    - [ ] 16.12 - UI: Variables in templates: {{name}}, {{email}}, {{date}} with preview
